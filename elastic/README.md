...
| Field | Description |
|-------|-------------|
| `projectId` | Project short ID (auto-set by `openkbs create`) |
...
| `mqtt` | `true` to enable real-time MQTT messaging |
...
> The Anthropic SDK sends `x-api-key` and the Google SDK sends `x-goog-api-key` — neither works with the proxy.
> You must add `headers: { Authorization: \`Bearer ${apiKey}\` }` when configuring Anthropic or Google providers.
